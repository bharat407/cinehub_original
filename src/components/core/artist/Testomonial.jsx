...
const MemberTestimonials = () => {
  return (
    <div className="bg-[#E9E9E9] py-8">
      <div className="max-w-5xl mx-auto">
        <h3 className="relative text-2xl font-bold text-center text-black mb-8">
          HERE IS WHAT OUR MEMBERS ARE SAYING
          <span className="absolute left-1/2 transform -translate-x-1/2 mt-2 w-40 h-1 bg-red-600 block"></span>
        </h3>

        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {testimonials.map((testimonial, index) => (
            <div key={index} className=" p-6 rounded  text-center text-black">
              <img
              ...
              <p className="text-sm text-black relative">
                <span className="absolute left-0 top-0 text-red-600">
                  <FaQuoteLeft />
                </span>
                {testimonial.quote}
                <span className="absolute right-0 bottom-0 text-red-600">
                  <FaQuoteRight />
                </span>
              </p>
            </div>
